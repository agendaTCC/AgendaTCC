/**
 * --------------------------------------------------------------------
 * jQuery-Plugin SuperBGimage - Scaling Fullscreen Backgrounds and Slideshow
 * using jQuery Version: 1.0, 29.08.2009
 * 
 * by Andreas Eberhard, andreas.eberhard@gmail.com
 * http://dev.andreaseberhard.de/projects/superbgimage/
 * 
 * Copyright (c) 2009 Andreas Eberhard licensed under a Creative Commons
 * Attribution 3.0
 * 
 * Inspired by Supersized - Fullscreen Slideshow jQuery Plugin
 * http://buildinternet.com/project/supersized/ By Sam Dunn
 * (www.buildinternet.com // www.onemightyroar.com)
 * -------------------------------------------------------------------- License:
 * http://creativecommons.org/licenses/by/3.0/
 * http://creativecommons.org/licenses/by/3.0/deed.de
 * 
 * You are free: * to Share - to copy, distribute and transmit the work * to
 * Remix - to adapt the work
 * 
 * Under the following conditions: * Attribution. You must attribute the work in
 * the manner specified by the author or licensor (but not in any way that
 * suggests that they endorse you or your use of the work).
 * --------------------------------------------------------------------
 * Changelog: 29.08.2009 initial Version 1.0
 * --------------------------------------------------------------------
 */
(function($) {
	jQuery.fn.superbgimage = function(loadingoptions) {
		var options = $.extend($.fn.superbgimage.defaults,
				$.fn.superbgimage.options, loadingoptions);
		...
		$('#' + options.id).css('display', 'none').css('overflow', 'hidden')
				.css('z-index', options.z_index);
		if (options.inlineMode === 0) {
			$('#' + options.id).css('position', 'fixed').css('width', '100%')
					.css('height', '100%').css('top', 0).css('left', 0);
		}
		if (options.reload) {
		...
		if ($.superbg_isIE6 && (options.inlineMode === 0)) {
			$('#' + options.id).css('position', 'absolute').width(
	...
	jQuery.fn.superbgLoad = function() {
		var options = $.extend($.fn.superbgimage.defaults,
				$.fn.superbgimage.options);
		if (($('#' + options.id).children('img').length > 0)
				|| ($('#' + $.superbg_saveId).children('a').length > 0)) {
			$('#' + options.id).show();
		}
		...
		if (options.randomimage === 1) {
			$.superbg_imgActual = (1 + parseInt(Math.random()
					* ($.superbg_imgIndex - 1 + 1), 10));
		}
	...
	jQuery.fn.superbgimagePreload = function() {
		...
		if (!$.superbg_firstLoaded
				&& ($('#' + $.superbg_saveId).children('a').length > 0)) {
			$.superbg_preload = setInterval("$(this).superbgimagePreload()",
					111);
			return;
		}
		$('#' + $.superbg_saveId)
				.children('a.preload:first')
				.each(
						function() {
							...
							$.superbg_preload = setInterval(
									"$(this).superbgimagePreload()", 111);
						}).removeClass('preload');
	};
	...
	jQuery.fn.nextSlide = function() {
		...
		if (options.randomimage === 1) {
			$.superbg_imgSlide = (1 + parseInt(Math.random()
					* ($.superbg_imgIndex - 1 + 1), 10));
	...
	jQuery.fn.superbgResize = function() {
		...
		var dimensions = $(this).superbgCalcSize($(thisimg).width(),
				$(thisimg).height());
	...
	jQuery.fn.superbgShowImage = function(img) {
		...
		if ($('#' + options.id).children(
				'img' + "[rel='" + $.superbg_imgActual + "']").length === 0) {
			imgsrc = $(
					'#' + $.superbg_saveId + ' a' + "[rel='"
							+ $.superbg_imgActual + "']").attr('href');
			imgtitle = $(
					'#' + $.superbg_saveId + ' a' + "[rel='"
							+ $.superbg_imgActual + "']").attr('title');
		} else {
			imgsrc = $('#' + options.id).children(
					'img' + "[rel='" + $.superbg_imgActual + "']").attr('src');
		}
		if ((typeof options.onHide === 'function') && (options.onHide !== null)
				&& ($.superbg_imgLast >= 0)) {
		...
		$('#' + $.superbg_saveId).children(
				'a' + "[rel='" + $.superbg_imgActual + "']").addClass(
				'activeslide');
		$.superbg_imgSlide = $.superbg_imgActual;
			...
			$(img)
					.load(
							function() {
								...
								var thisimg = $('#' + options.id).children(
										'img' + "[rel='" + $.superbg_imgActual
												+ "']");
	...
	jQuery.fn.superbgTransition = function(thisimg, dimensions) {
		...
		var newtop = dimensions[3];
		$(thisimg).css('width', newwidth + 'px')
				.css('height', newheight + 'px').css('left', newleft + 'px');
		...
		if (akt_transitionout === 1) {
			$('#' + options.id + ' img.activeslide').removeClass('activeslide')
					.addClass('lastslide').css('z-index', 0);
		} else {
			$('#' + options.id + ' img.activeslide').removeClass('activeslide')
					.addClass('lastslideno').css('z-index', 0);
		}
			...
			$(thisimg).fadeIn(
					...
					function() {
						if ((typeof options.onShow === 'function')
								&& (options.onShow !== null))
							options.onShow($.superbg_imgActual);
						$('#' + options.id + ' img.lastslideno').hide(1, null)
								.removeClass('lastslideno');
						$.superbg_inAnimation = false;
						if ($.superbg_slideshowActive) {
							$('#' + options.id).startSlideShow();
						}
		...
		if (akt_transitionout === 1) {
			...
			if (options.transition === 0) {
				$('#' + options.id + ' img.lastslide').hide(1, null)
						.removeClass('lastslide');
			} else if (options.transition == 1) {
				$('#' + options.id + ' img.lastslide').fadeOut(outspeed)
						.removeClass('lastslide');
			} else {
				if (options.transition === 2) {
					theEffect = 'slide';
			...
			$('#' + options.id + ' img.lastslide').hide(1, null).removeClass(
					'lastslide');
		}
	};
